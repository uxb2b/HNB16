...
namespace ModelCore.Service
{
	...
	public static class OutboundSvc
	{
		...
		{
			string uri = new System.Text.StringBuilder(Settings.Default.urlToCDS)
				.Append("?type=LCRN&fileName=").Append(HttpUtility.UrlEncode("Advice-"+ item.AmendmentNo + ".xml"))
				.Append("&no=").Append(HttpUtility.UrlEncode(item.AmendmentNo)).ToString();
        ...
        public static void SendRejection(Documentary rejectData, string reason)
        {
            ...
            switch ((Naming.DocumentTypeDefinition)rejectData.DocType)
            {
                case Naming.DocumentTypeDefinition.開狀申請書:
                    dsAppl.Type = "LCA";
                    docNo = rejectData.CreditApplicationDocumentary.ApplicationNo;
                    dsAppl.ApplyLcNo.ApplNo = rejectData.CreditApplicationDocumentary.ApplicationNo;
                    break;
