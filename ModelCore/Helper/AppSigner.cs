...
namespace ModelCore.Helper
{
    public class AppSigner
    {
        ...
        private String certPath
        {
            get
            {
                return Path.Combine(CommonLib.Core.Utility.Logger.LogPath, __CERT_FILE);
            }
        }
            ...
            if (File.Exists(certPath))
            {
                using (FileStream fs = new FileStream(certPath, FileMode.Open, FileAccess.Read))
                {
                    using (MemoryStream ms = new MemoryStream())
                    {
                        ...
                        _signerCert = new X509Certificate2(certBytes, (string)null, X509KeyStorageFlags.Exportable);
                        ms.Close();
                    }
                    fs.Close();
                }
        ...
        public static bool Sign(XmlDocument docMsg)
        {
            ...
            {
                return CryptoUtility.SignXmlSHA256(docMsg, null, null, AppSigner.SignerCertificate);
            }
            return false;
        }
